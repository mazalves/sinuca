...
void branch_predictor_t::allocate() {
    ...
    switch (this->get_branch_predictor_type()) {
        case BRANCH_PREDICTOR_TWO_LEVEL:
...
processor_stage_t branch_predictor_t::predict_branch(opcode_package_t actual_opcode, opcode_package_t next_opcode) {
    ...
    if (actual_opcode.opcode_operation != INSTRUCTION_OPERATION_BRANCH) {
        ...
        switch (this->get_branch_predictor_type()) {
            ...
            case BRANCH_PREDICTOR_STATIC_TAKEN: {
                bool is_btb_hit = this->btb_find_update_address(actual_opcode.opcode_address);
            ...
            default:
                ERROR_PRINTF("Invalid branch predictor type %u.\n", this->get_branch_predictor_type());
            break;
        }
